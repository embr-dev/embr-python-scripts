# PySequence Attributes

公式 Help の Attributes ページを原文寄りに整理したもの。

| 項目 | 内容 |
|------|------|
| 一次情報 | `docs/API Documentation/Attributes/PySequence Attributes/` |
| 関連 | [flame-module.md](../flame-module.md)（メソッド索引） |

---

### `name`

- **Type:** `str`
- **Description:** The name of an object in the Media Panel, resolving tokens if any are present.
- **Value range:** None

### `uid`

- **Type:** `str`
- **Description:** The unique identifier of an object in the Media Panel.
- **Value range:** None

### `token_name`

- **Type:** `str`
- **Description:** The tokenized name of an object in the Media Panel.
- **Value range:** None

### `expanded`

- **Type:** `bool`
- **Description:** The expanded state of an object in the Media Panel. If you're expanding/collapsing a Shared Library, you must have exclusive access if you want this property to be persisted.
- **Value range:** True · False

### `colour`

- **Type:** `tuple`
- **Description:** The colour of an object in the Media Panel.
- **Value range:** (0.0, 0.0, 0.0) · (1.0, 1.0, 1.0)

### `colour_label`

- **Type:** `str`
- **Description:** The colour label of an object in the Media Panel.
- **Value range:** None

### `selected`

- **Type:** `bool`
- **Description:** The PyArchiveEntry is selected in the Media Panel.
- **Value range:** True · False

### `start_time`

- **Type:** `flame.PyTime`
- **Description:** The Clip's start time, as a PyTime object.
- **Value range:** None

### `in_mark`

- **Type:** `NoneType`
- **Description:** The Clip's In mark. Undefined until set. Set as a timecode, or a frame number relative to the start of the clip (1 based). The mark is exclusive or inclusive, depending on the application Preference.
- **Value range:** None

### `out_mark`

- **Type:** `NoneType`
- **Description:** The Clip's out mark. Undefined until set. Set as a timecode, or a frame number relative to the start of the clip (1 based). The mark is exclusive or inclusive, depending on the application Preference.
- **Value range:** None

### `current_time`

- **Type:** `flame.PyTime`
- **Description:** The position of the playhead on the Clip.
- **Value range:** None

### `primary_track`

- **Type:** `flame.PyTrack`
- **Description:** The Primary Track of the Sequence.
- **Value range:** None

### `secondary_track`

- **Type:** `NoneType`
- **Description:** The Secondary Track of the Sequence.
- **Value range:** None

### `selected_segments`

- **Type:** `list`
- **Description:** Return a list of PySegment currently selected.
- **Value range:** None

### `selected_markers`

- **Type:** `list`
- **Description:** Return a list of PyMarker currently selected.
- **Value range:** None

### `tags`

- **Type:** `list`
- **Description:** The tag list of an object in the Media Panel.
- **Value range:** None Parent page: Autodesk Flame Python: flame module

###
